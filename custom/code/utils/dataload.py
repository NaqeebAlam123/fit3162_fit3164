...
class SER_MFCC(data.Dataset):
    def __init__(self,
      ...
      #  file.close()
        ...
        self.train = train
        if(self.train=='train'):
            file = open('emotion_pretrain/train_M030.pkl', "rb") #'rb'-read binary file
            self.train_data = pickle.load(file)
            file.close()
        if(self.train=='val'):
            file = open('emotion_pretrain/val_M030.pkl', "rb") #'rb'-read binary file
            self.train_data = pickle.load(file)
            file.close()
    ...
    def __getitem__(self, index):
        ...
        file.close()
        if mfcc.shape == torch.Size([1, 25, 12]):
            mfcc = pad(input=mfcc, pad=(0, 0, 0, 3), mode='constant', value=0)
        return mfcc, label
