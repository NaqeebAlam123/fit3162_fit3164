...
from torch.utils.data import \
    DataLoader  # for easier dataset management (creation of batches to train and validate data on)
import torch.utils
import torchvision
import numpy as np
...
def copy_params(pretrain, tgt_state,model_name,strip):
    """
    This function copy parameters in pre trained models to our current model.state dict is Python like dictionary
    which is used to encapsulate and map each layer to its parameter tensor which includes weight and biases values
    Parameters
    ----------
    pretrain
    tgt_state
...
def load_data(config,directory,model):
    if model=="Auto_Encoder":
     train_set = GET_MFCC(directory, 'train')
     test_set = GET_MFCC(directory, 'test')
    elif model=="Emotion_Net":
     train_set = SER_MFCC(directory, 'train')
     test_set = SER_MFCC(directory, 'val')
    elif model=="AT_Emotion":
...
def train_test_batches_single_epoch(cross_entropy_loss, config, epoch, model, optimizer, start_train_time,
    ...
    for batch_id_x, (data, target_label) in enumerate(data_loader):
        ...
        if (iteration_val % 1000 == 0):
            ...
            print(template.substitute(epoch=epoch + 1, batch_id=batch_id_x, data_Size=len(data_loader), mode=mode,
                                      loss=loss.item(), time=time_taken))
...
def train_EmotionNet(config):
    ...
    test_loader,train_loader=load_data(config,EMOTION_NET_DATASET_DIR,"Emotion_Net")
    print('load data end')
    ...
    for epoch in range(config.max_epochs_val):
        ...
        (model, train_iteration_val) = train_test_batches_single_epoch(cross_entropy_loss, config, epoch, model,
                                                                       optimizer, start_train_time,
                                                                       train_iteration_val, train_loader, writer,
                                                                       "Train", t)


        path_val = os.path.join(EMOTION_NET_MODEL_DIR,
                                'SER_' + str(epoch) + '.pkl')  ## NOTE: where SER_99.pkl comes from
        ...
        with torch.no_grad():
            ...
            val_iteration_val = train_test_batches_single_epoch(cross_entropy_loss, config, epoch, model, optimizer,
                                                                start_train_time, val_iteration_val, val_loader, writer,
                                                                "Test", t)
...
def train_AutoEncoder2x(config):
    ...
    if config.pretrain:
        ...
        strip = "module."
        tgt_state=copy_params(pretrain, tgt_state,'con_encoder',strip)

        # SER resume
        pretrain = torch.load(SERPRETRAINED_DIR)
        # tgt_state = model.state_dict()

        tgt_state=copy_params(pretrain,tgt_state,'emo_encoder',strip)

        tgt_state=copy_params(pretrain, tgt_state, 'classify',strip)

    # ------- 2. Load training data -------#
    print('start split')
    test_loader, train_loader = load_data(config,AUTOENCODER_2X_DATASET_DIR,"Auto_enocder")
    print('end split')
    ...
    for epoch in range(start_epoch, config.max_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if (train_iter % 100 == 0):  # 500
                with open(AUTOENCODER_2X_LOG_DIR + 'train.txt', 'a') as file_handle:
                    file_handle.write(
                        '[%d,%5d / %d] con_feature loss :%.10f cla_1 loss :%.10f train loss :%.10f time spent: %f s' % (
                        epoch + 1, i + 1, len(train_loader), losses['con_feature'].item(), losses['cla_1'].item(),
                        loss.item(), time.time() - a))
                    file_handle.write('\n')
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                ...
                if (val_iter % 10 == 0):
                    print('[%d,%5d / %d] con_feature loss :%.10f cla_1 loss :%.10f val loss :%.10f time spent: %f s' % (
                    epoch + 1, i + 1, len(test_loader), losses['con_feature'].item(), losses['cla_1'].item(),
                    loss.item(), time.time() - a))
                ...
                if (val_iter % 500 == 0):  # 2000
                    ...
                    with open(AUTOENCODER_2X_LOG_DIR + 'val.txt', 'a') as file_handle:
                        file_handle.write(
                            '[%d,%5d / %d] con_feature loss :%.10f cla_1 loss :%.10f val loss :%.10f time spent: %f s' % (
                            epoch + 1, i + 1, len(test_loader), losses['con_feature'].item(), losses['cla_1'].item(),
                            loss.item(), time.time() - a))
                        file_handle.write('\n')
...
def train_AT_Emotion(config):
    """ 1. load the data """
    print('start split')
    test_loader,train_loader=load_data(config,LM_ENCODER_DATASET_LANDMARK_DIR,"AT_Emotion")
    print('end split')
    ...
    if config.pretrain:
        ...
        copy_params(pretrain,tgt_state,)
        strip = 'con_encoder.'
        tgt_state=copy_params(pretrain, tgt_state,strip,strip )
    ...
    pca = np.load(LANDMARK_BASICS+'U_68.npy')[:, :16]
    mean = np.load(LANDMARK_BASICS+'mean_68.npy')
